...
#
# Author:
#   zhanyi(zyzy257@gmail.com)
# fork: hubot-scripts/hubot-jenkins
...
#    job = querystring.escape msg.match[1]
    ...
    jobUrl = _getJobUrlByName(job)

    path = if params then "#{jobUrl}/buildWithParameters?#{params}" else "#{jobUrl}/#{command}"
    ...
    req.post() (err, res, body) ->
        if err
          ...
          msg.reply "(#{res.statusCode}) Build started for #{job} #{jobUrl}/"
    ...
    req.get() (err, res, body) ->
        if err
          ...
          try
            ...
            path = "#{jobUrl}/#{content.lastBuild.number}/api/json"
            req = msg.http(path)
            if process.env.HUBOT_JENKINS_AUTH
              auth = new Buffer(process.env.HUBOT_JENKINS_AUTH).toString('base64')
              req.headers Authorization: "Basic #{auth}"
...
jenkinsLast = (msg) ->
    ...
    path = "#{jobUrl}/lastBuild/api/json"
...
_getJobNameByUrl =  (url) ->
    prefixUrl = process.env.HUBOT_JENKINS_URL + "/job/"
    jobName = url.replace(prefixUrl, "").split('/job/').join("::").replace("/","")
    return jobName

_getJobUrlByName = (jobName) ->
    prefixUrl = process.env.HUBOT_JENKINS_URL + "/job/"
    jobUrl = prefixUrl + jobName.split("::").join("/job/")
# 不要最后的斜线，否则会404
#    jobUrl = prefixUrl + jobName.split('::').join("/job/") + "/"
    return jobUrl


_getJenkinsJobs = (msg, url, filter) ->
    req = msg.http("#{url}/api/json?jobs[name,url,jobs,color]")
          ...
          try
            ...
            for job in content.jobs
              ...
              if job._class == "com.cloudbees.hudson.plugins.folder.Folder"
                  jobUrl = job.url.substring(0,job.url.length-1)
                  _getJenkinsJobs(msg, jobUrl, filter)
                  continue

              jobName = _getJobNameByUrl(job.url)

              index = jobList.indexOf(jobName)
              if index == -1
              ...
              if filter.test job.name
                response += "[#{index + 1}] #{state} #{jobName}\n"
